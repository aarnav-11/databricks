...
| UC metric view | `dmpipeline-dev.ri_gold.claim_fraud_metrics` | `claim_fraud_metrics` | `CLAIM_FRAUD_METRICS_TABLE` |
...
| `fraud_metrics` | `claim_fraud_metrics` | Queries `siu_referral_rate` and `avg_fraud_score` with `MEASURE()`; mandatory baseline |
| `party_profile` | `party_360` | Uses related party IDs, `policy_number`, or a claim ID/number available in the table |
| `location_profile` | `location_360` | Uses related location/address IDs found in earlier evidence |
| `policy_profile` | `policy_360` | Uses `policy_id`, `policy_number`, or a claim ID/number available in the table |
...
The MCP adapter calls `list_tools()` at runtime. When Ontobricks exposes
`list_domains` and `select_domain`, the adapter runs both before the claim
...
rejects write-like names, and invokes a tool only when its required inputs can
be populated from the question and claim ID. If no compatible tool exists, the
trace lists the discovered names and asks for `MCP_TOOL_NAME`; it does not guess
...
Supported `kind` values are `table`, `metric_view`, `vector_search`, and `mcp`.
Metric-view planes also declare `measure_columns`; every configured measure is
queried through `MEASURE()` rather than `SELECT *`. An entry with
