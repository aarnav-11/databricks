...
SUPPORTED_KINDS = frozenset({"table", "metric_view", "vector_search", "mcp"})
...
DEFAULT_PLANE_SPECS: tuple[PlaneSpec, ...] = (
    ...
    PlaneSpec(
        ...
        "table",
        "POLICY_TABLE",
        ("policy_id", "policy_number", "claim_id", "claim_number"),
    ),
    PlaneSpec(
        "document_search",
...
def _spec_from_dict(payload: Mapping[str, Any]) -> PlaneSpec:
    ...
    measure_columns = tuple(str(value) for value in payload.get("measure_columns", ()))
    return PlaneSpec(
        name=name,
