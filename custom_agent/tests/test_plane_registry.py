...
class PlaneRegistryTests(unittest.TestCase):
    ...
    def test_defaults_match_linked_app_resources(self):
        ...
        self.assertEqual(specs["fraud_metrics"].kind, "metric_view")
        self.assertEqual(
            specs["fraud_metrics"].measure_columns,
            ("siu_referral_rate", "avg_fraud_score"),
        )
        self.assertIn("policy_number", specs["policy_profile"].lookup_columns)
        self.assertIn("policy_number", specs["party_profile"].lookup_columns)
