...
| UC metric view | `dmpipeline-dev.ri_gold.claim_fraud_metrics` | **Can select** | `claim_fraud_metrics` |
...
| `fraud_metrics` | `claim_id`; measures `siu_referral_rate`, `avg_fraud_score` |
| `party_profile` | `party_id`, `claimant_party_id`, `insured_party_id`, `policy_number`, `claim_id`, `claim_number` |
| `location_profile` | `location_id`, `loss_location_id`, `address_id`, `claim_id` |
| `policy_profile` | `policy_id`, `policy_number`, `claim_id`, `claim_number` |
...
- `claim_profile` shows `query_table`, and `fraud_metrics` shows
  `query_metric_view` with explicit `MEASURE()` expressions;
...
The supervisor first calls MCP `list_tools()`. If Ontobricks exposes
`list_domains` and `select_domain`, it runs those before each graph lookup. A
single domain, or one unique domain containing `claim`, `insurance`, or `fraud`
in its metadata, is selected automatically.

If the trace lists multiple `available_domains`, add this plain environment
value directly below `MCP_APP_NAME` in `custom_agent/app.yaml`, using the exact
claims-domain name shown in the trace:

```yaml
  - name: MCP_DOMAIN_NAME
    value: exact-claims-domain-name
```
...
`metric_view`, `vector_search`, and `mcp`. A `metric_view` entry must also
provide `measure_columns`.
